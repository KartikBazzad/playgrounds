...
import { getDB, getConn } from '@/lib/duckdb';
...
// Provide stubs for DuckDB: getDB for ready state, getConn for shared connection actions
const mockConn = { query: vi.fn(async () => ({})), close: vi.fn(async () => {}) } as any;
vi.mock('@/lib/duckdb', () => ({
  getDB: vi.fn(async () => ({ connect: vi.fn(async () => mockConn) })),
  getConn: vi.fn(async () => mockConn),
}));
...
describe('Playground flows', () => {
  ...
  it('vars panel: add/rename/remove and interpolation used on Run', async () => {
    ...
    render(<Playground />);
    // Show vars
    await user.click(await screen.findByText('Show Vars'));
    // Add a var
    await user.click(await screen.findByText('Add Var'));
  ...
  it('more actions: Load Sample success shows message and refreshes schema', async () => {
    const user = userEvent.setup();
    idbGetMock.mockResolvedValue('0');
    const okConn = { query: vi.fn(async () => ({})), close: vi.fn(async () => {}) } as any;
    (getConn as unknown as { mockResolvedValueOnce: any }).mockResolvedValueOnce(okConn);
    render(<Playground />);
    const more = await screen.findByLabelText('More actions');
    await user.click(more);
    await user.click(await screen.findByText('Load Sample'));
  ...
  it('more actions: Load Sample error path shows error', async () => {
    const user = userEvent.setup();
    idbGetMock.mockResolvedValue('0');
    (getConn as unknown as { mockRejectedValueOnce: any }).mockRejectedValueOnce(new Error('sample fail'));
    render(<Playground />);
    const more = await screen.findByLabelText('More actions');
    await user.click(more);
    await user.click(await screen.findByText('Load Sample'));
  ...
  it('more actions: install httpfs success shows message', async () => {
    ...
    const more = await screen.findByLabelText('More actions');
    await user.click(more);
    await user.click(await screen.findByText('Install httpfs'));
  ...
  it('more actions: Parquet demo success shows rows and message', async () => {
    ...
    (getConn as unknown as { mockResolvedValueOnce: any }).mockResolvedValueOnce({ query, close: vi.fn(async () => {}) });

    render(<Playground />);
    const more = await screen.findByLabelText('More actions');
    await user.click(more);
    await user.click(await screen.findByText('Parquet demo'));
  ...
  it('more actions: Parquet demo error path shows error', async () => {
    const user = userEvent.setup();
    idbGetMock.mockResolvedValue('0');
    // getConn rejects on action
    (getConn as unknown as { mockRejectedValueOnce: any }).mockRejectedValueOnce(new Error('parquet fail'));
  ...
  it('more actions: Reset DB error path shows error', async () => {
    ...
    idbGetMock.mockResolvedValue('0');
    // getConn rejects on action
    (getConn as unknown as { mockRejectedValueOnce: any }).mockRejectedValueOnce(new Error('reset fail'));
    render(<Playground />);
    const more = await screen.findByLabelText('More actions');
    await user.click(more);
    await user.click(await screen.findByText('Reset'));
  ...
  it('datasets: load each dataset shows message and refreshes schema', async () => {
    ...
    // People (local CSV)
    await user.click(openDatasets);
    await user.click(await screen.findByText('People (local CSV)'));
    await waitFor(() => expect(screen.getByText('Loaded dataset: people (local CSV)')).toBeTruthy());
    // Tips (remote CSV)
    await user.click(openDatasets);
    await user.click(await screen.findByText('Tips (remote CSV)'));
    await waitFor(() => expect(screen.getByText('Loaded dataset: tips (remote CSV)')).toBeTruthy());
    // TPCH Lineitem (Parquet) menu entry
    await user.click(openDatasets);
    await user.click(await screen.findByText('TPCH Lineitem (Parquet)'));
  ...
  it('copy SQL copies current editor text', async () => {
    ...
    await waitFor(() => expect(screen.getByText('SQL copied to clipboard')).toBeTruthy());
  });
  ...
  it('download CSV enabled after run and triggers blob URL', async () => {
    ...
    // Prevent jsdom navigation warning by stubbing anchor click
    const anchorClickSpy = vi.spyOn(HTMLAnchorElement.prototype as any, 'click').mockImplementation(() => {});
  ...
  it('outside-click closes menus: More actions, Snippets, and Datasets', async () => {
    ...
    // Snippets
    const sn = await screen.findByLabelText('Snippets');
    await userEvent.setup().click(sn);
    ...
    await userEvent.setup().click(ds);
  ...
  it('notebook: applySnippet updates active cell (not global) and runs', async () => {
    ...
    await userEvent.setup().click(sn);
    const opt = await screen.findByRole('button', { name: /load sample/i });
    await userEvent.setup().click(opt);
    ...
    const runAll = screen.getByLabelText('Run all');
    await userEvent.setup().click(runAll);
    await waitFor(() => expect(runMock).toHaveBeenCalled());
  });
  ...
  it('notebook: runAll with error sets cell message and clears running', async () => {
    idbGetMock.mockResolvedValue(undefined);
    // First cell resolves, second rejects
    runMock
      .mockResolvedValueOnce({ columns: [], rows: [], execMs: 1, message: 'ok' })
      .mockRejectedValueOnce(new Error('cell fail'));
    render(<Playground />);
    // Add a second cell and set SQLs
    const addBtn = await screen.findByLabelText('Add cell below');
    await userEvent.setup().click(addBtn);
    await waitFor(() => expect(screen.getAllByLabelText('cm').length).toBe(2));
    const cms = screen.getAllByLabelText('cm') as HTMLTextAreaElement[];
    ...
    await userEvent.setup().click(runAll);
    await waitFor(() => expect(runMock).toHaveBeenCalledTimes(2));
    // Switch second cell to Messages tab and assert error message
    const msgTabs = screen.getAllByRole('button', { name: /messages/i });
    // second cell is the second tab button in order
    await userEvent.setup().click(msgTabs[1]);
    await waitFor(() => expect(screen.getByText(/cell fail/i)).toBeTruthy());
  });

  it('notebook: quality report applies to active cell and triggers run', async () => {
    const user = userEvent.setup();
    idbGetMock.mockResolvedValue(undefined);
    runMock.mockResolvedValue({ columns: [], rows: [], execMs: 1, message: 'ok' });
    render(<Playground />);
    ...
    const item = await screen.findByText(/quality report/i);
    await user.click(item);
    // Active cell should update with the quality report SQL
    const cm = await screen.findByLabelText('cm');
    await waitFor(() => expect((cm as HTMLTextAreaElement).value).toMatch(/Data quality report/));
    // And run should be triggered (debounced via setTimeout)
    await waitFor(() => expect(runMock).toHaveBeenCalled());
  });
});
